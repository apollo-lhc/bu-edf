...
int main(){
  ...
  try {
	...
	if (sensor_type == "IpmiTemperature") {
      	  sensors[wsp_filename] = new IpmiTemperatureSensor(ipmi_sensor_number, wsp_filename);
	  sensors[wsp_filename]->Connect(IP_addr, port_number);
	} else if (sensor_type == "IpmiFanspeed") {
	  sensors[wsp_filename] = new IpmiFanSpeedSensor(ipmi_sensor_number, wsp_filename);
	  sensors[wsp_filename]->Connect(IP_addr, port_number);
	} else {
	...
	for ( std::pair<std::string, Sensor*> sensor : sensors ) {
