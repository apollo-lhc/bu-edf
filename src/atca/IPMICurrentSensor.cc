...
#include <atca/IPMICurrentSensor.hh>
#include <stdexcept>
#include <string.h>
#include <atca/IPMBSensorResult.hh>

IPMICurrentSensor::IPMICurrentSensor(std::vector<std::string> const & args){
  /*
    args:
      sensor name
      base
  ...
  */
  ...
  SetDatabaseName(args[1] + ".IPMICurrenterature" + "." + args[0]);
...
void IPMICurrentSensor::SetSensorNumber(std::string const & val){
  //check if val is a digit
  sensorNumber = atoi(val.c_str());
}

void IPMICurrentSensor::SetHostname(std::string const & val){
  hostname = val;
}

void IPMICurrentSensor::SetDeviceAccessAddress(std::string const & val){
  //check if val is a digit
  deviceAccessAddress = strtoul((char const *) val.c_str(), NULL, 0);
}


float IPMICurrentSensor::GetVal(){
 ...
 // net_fn code for "Sensor Events" 0x04
  ...
  buf_rq[0] = 0x2d;
  // sensor number (235-238 for tray Currents, 240-243 for fan speeds, etc)
